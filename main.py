...
def plot_stock_analysis(symbol, df, show_volume=True):
    ...
    try:
        ...
        # CHỈ TÍNH RS NẾU MÃ KHÔNG PHẢI LÀ VNINDEX
        if symbol.upper() != "VNINDEX":
            try:
             ...
             # Nếu là VNINDEX, gán các giá trị mặc định cho RS để tránh lỗi
            df["RS"] = 1.0
            df["RS_Point"] = 0.0
            df["RS_SMA_10"] = 1.0
            df["RS_SMA_20"] = 1.0
        ...
        try:
            ...
            # CHỈ VẼ RS NẾU MÃ KHÔNG PHẢI LÀ VNINDEX
            if symbol.upper() != "VNINDEX":
                ...
                plt.legend(fontsize=7, loc="upper left")
                 ...
                 # Nếu là VNINDEX, bỏ qua 2 subplot RS và RS_Point
                 # Có thể thêm subplot trống hoặc giảm số lượng subplot nếu cần, ở đây chỉ bỏ qua
                 pass
        ...
        try:
            ...
            # 6. RS (Relative Strength) - 10 điểm (BỎ QUA NẾU LÀ VNINDEX)
            if symbol.upper() != "VNINDEX":
                rs_score = 0
                ...
                if rs_value > last_row.get("RS_SMA_50", rs_value):
                    rs_score += 5
                score += rs_score
            # 7. RS_Point - 10 điểm (BỎ QUA NẾU LÀ VNINDEX)
            if symbol.upper() != "VNINDEX":
                rs_point_score = 0
...
"""
        # CHỈ THÊM RS VÀO PROMPT NẾU MÃ KHÔNG PHẢI LÀ VNINDEX
        if symbol.upper() != "VNINDEX":
            prompt += f"- RS (Amibroker): {rs_val} (SMA10: {rs_sma10_val}) (SMA20: {rs_sma20_val}) (SMA50: {rs_sma50_val}) (SMA200: {rs_sma200_val})\n"
            prompt += f"- RS_Point: {rs_point_val} (SMA10: {rs_point_sma10_val}) (SMA20: {rs_point_sma20_val}) (SMA50: {rs_point_sma50_val}) (SMA200: {rs_point_sma200_val})\n"
...
def analyze_stock(symbol):
    ...
    # CHỈ HIỂN THỊ RS NẾU MÃ KHÔNG PHẢI LÀ VNINDEX
    if symbol.upper() != "VNINDEX":
