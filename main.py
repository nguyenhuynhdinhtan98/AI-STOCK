...
def calculate_technical_score(df, symbol):
    ...
    try:
        ...
        if "Volume" in last_row and not pd.isna(last_row["Volume"]):
            ...
            # Giới hạn điểm volume
            volume_score = np.clip(volume_score, -14, 14)

        score += volume_score

        # 6. RS (Relative Strength) & RS_Point - 14 điểm
        if symbol.upper() != "VNINDEX":
            ...
            if rs_point_value > 1.0:
                rs_score += 3.5
            elif rs_point_value < -1.0:
                rs_score -= 3.5

            score += rs_score

        # 7. Bollinger Bands - 14 điểm
        bb_score = 0
